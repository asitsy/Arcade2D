...
public class CollisionManager
{
    ...
    // LINQ перевіряє, чи перетинається переданий прямокутник із будь-якою стіною
    ...
    {
        // 1. Зіткнення з привидами (якщо вони не заморожені)
        if (freezeTimer <= 0f)
        {
        ...
        // 2. Збір супер-пелетів (PowerPellet) тепер ПЕРШИМ!
        ...
        if (collidedPowerPellet != null)
        {
            ...
            score += 10;
            freezeTimer = 5f; // Заморожуємо привидів на 5 секунд
            return false; // Повертаємось, щоб не обробляти його як звичайний пелет
        }

        // 3. Збір звичайних пелетів (Pellet) за допомогою LINQ
